...
#include "nav2_behavior_tree/plugins/action/crate_search_action.hpp"

namespace nav2_behavior_tree
{
CrateSearchAction::CrateSearchAction(
  const std::string & xml_tag_name,
  const std::string & action_name,
  const BT::NodeConfiguration & conf)
: BtActionNode<nav2_msgs::action::Wait>(xml_tag_name, action_name, conf) //TODO change wait action
{
